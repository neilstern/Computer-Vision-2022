...
def grid_points(img, nPointsX, nPointsY, border):
    ...
    # todo
    vPoints = np.empty([nPointsX*nPointsY, 2])
    h = img.shape[0]
    w = img.shape[1]
    hdist = (h - 2 * border) / (nPointsY - 1) # x points -> x-1 segments
    wdist = (w - 2 * border) / (nPointsX - 1)
    for i in range(nPointsY):
        for j in range(nPointsX):
            vPoints[nPointsX * i + j] = [hdist * i + border, wdist * j + border]
    return vPoints
    ...
    for i in range(len(vPoints)):
        ...
        for cell_y in range(-2, 2):
            for cell_x in range(-2, 2):
                ...
                # todo
                # compute the angles
                # magnitudes, angles = cv2.cartToPolar(grad_x, grad_y, True)
                # magnitudes = np.sqrt(np.power(grad_x, 2) + np.power(grad_y, 2))
                angles = np.arctan2(grad_y, grad_x) * (180 / np.pi) + 180 # in range [0, 360]

                # compute the histogram
                hog_cell = np.zeros(nBins)
                for i in range(start_x, end_x):
                    for j in range(start_y, end_y):
                        bin_idx = int(angles[i][j] * nBins / 360.0)
                        if bin_idx == nBins: # edgecase where angle is 360 degrees 
                            bin_idx -= 1
                        hog_cell[bin_idx] += 1; # += magnitudes[i][j]
                desc.append(hog_cell)
...
def create_codebook(nameDirPos, nameDirNeg, k, numiter):
    ...
    for i in tqdm(range(nImgs)):
        ...
        vFeatures.append(descriptors_hog(img, grid, cellWidth, cellHeight))
...
def create_bow_histograms(nameDir, vCenters):
    ...
    for i in tqdm(range(nImgs)):
        ...
        # todo
        grid = grid_points(img, nPointsX, nPointsY, border)
        vBoW.append(bow_histogram(descriptors_hog(img, grid, cellWidth, cellHeight), vCenters))
