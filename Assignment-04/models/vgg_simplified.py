...
class Vgg(nn.Module):
    def __init__(self, fc_layer=512, classes=10):
        ...
        self.conv_block5 = nn.Sequential(
            nn.Conv2d(512, fc_layer, 3, padding=1),
            nn.MaxPool2d(2)
        )
        self.linear1 = nn.Linear(fc_layer, 128)
    ...
    def forward(self, x):
        ...
        act_fn = nn.ReLU()
        x = act_fn(self.conv_block1(x))
        x = act_fn(self.conv_block2(x))
        x = act_fn(self.conv_block3(x))
        x = act_fn(self.conv_block4(x))
        x = act_fn(self.conv_block5(x))
        x = torch.flatten(x, 1)
