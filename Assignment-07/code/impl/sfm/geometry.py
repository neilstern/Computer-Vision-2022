...
def EstimateEssentialMatrix(K, im1, im2, matches):
  ...
  normalized_kps1 = (np.linalg.inv(K) @ im1.image[im1.kps[:, 1].astype(int), im1.kps[:, 0].astype(int)].T).T
  normalized_kps2 = (np.linalg.inv(K) @ im2.image[im2.kps[:, 1].astype(int), im2.kps[:, 0].astype(int)].T).T
  ...
  for i in range(matches.shape[0]):
    # TODO
    # Add the constraints
    p1 = im1.image[int(im1.kps[matches[i,0]][1]), int(im1.kps[matches[i,0]][0])]
    p2 = im2.image[int(im2.kps[matches[i,1]][1]), int(im2.kps[matches[i,1]][0])]
    constraint_matrix[i] = [p1[0] * p2[0], p1[0] * p2[1], p1[0], p1[1] * p2[0], p1[1] * p2[1], p1[1], p2[0], p2[1], 1]
...
def TriangulatePoints(K, im1, im2, matches):
  ...
  idx = [i for i in range(points3D.shape[0]) if (P1 @ points3D.T)[i, 2] > 0]
  im1_corrs = im1_corrs[i]
  im2_corrs = im2_corrs[i]
  points3D = points3D[i]

  # Filter points behind the second camera
  im1_corrs = im1_corrs[(P2 @ points3D)[2] > 0]
  im2_corrs = im2_corrs[(P2 @ points3D)[2] > 0]
  points3D = points3D[(P2 @ points3D)[2] > 0]
...
def EstimateImagePose(points2D, points3D, K):  
  ...
  normalized_points2D = np.linalg.inv(K) * points2D
...
def TriangulateImage(K, image_name, images, registered_images, matches):
  ...
  for img in registered_images:
    normalized_kps1 = np.linalg.inv(K) * img
    normalized_kps2 = np.linalg.inv(K) * image[image_name]
