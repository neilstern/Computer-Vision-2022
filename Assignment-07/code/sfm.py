...
def main():
  ...
  for i in range(len(possible_relative_poses)):
    e_im1.SetPose(possible_relative_poses[i][0], possible_relative_poses[i][1])
    e_im2.SetPose(possible_relative_poses[i][0], possible_relative_poses[i][1])
    p, _, _ = TriangulatePoints(K, e_im1, e_im2, e_matches)
    if len(p) > max_points:
      max_points = len(p)
      best_pose = i
  ...
  e_im1.SetPose(possible_relative_poses[best_pose][0], possible_relative_poses[best_pose][1])
