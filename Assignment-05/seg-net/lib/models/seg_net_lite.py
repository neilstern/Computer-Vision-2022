...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        layers_conv_down = [nn.Conv2d(input_size if i <= 0 else down_filter_sizes[i - 1], down_filter_sizes[i], kernel_sizes[i], padding=conv_paddings[i]) for i in range(self.num_down_layers)]
        ...
        layers_conv_up = [nn.Conv2d(down_filter_sizes[-1] if i <= 0 else up_filter_sizes[i - 1], up_filter_sizes[i], kernel_sizes[i], padding=conv_paddings[i]) for i in range(self.num_up_layers)]
        ...
        self.layer_conv_1 = nn.Conv2d(up_filter_sizes[-1], 11, 1)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_up_layers):
            x = self.layers_unpooling[i](x, idx_stack.pop())
